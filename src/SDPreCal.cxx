...
TGraph* SDPreCal::calcPreCal(std::vector<double> source_lines, std::vector<double> data_lines) {
	...
	cerr << "no. source lines: " << m_source_size
		<<"\nno. data lines: " << m_data_size
		<< endl;
	
	pair<SDPreCal::Mapping, SDPreCal::Stats> best_mapping;
	pair<SDPreCal::Mapping, SDPreCal::Stats> curr_mapping;
	Mapping startMap;
	...
	cerr << "n_start_vals: " << n_start_vals << endl;
	cerr << "best match: " << calcError(best_mapping.second) << endl;
	cerr << "rel. Error: " << best_mapping.second.sigma() / best_mapping.second.mean() << endl;
	...
	cerr << "best_mapping.first.size(): " << best_mapping.first.size() << endl;
